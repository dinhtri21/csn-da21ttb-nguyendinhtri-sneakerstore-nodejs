...
const addProduct = (req, res) => {
  try {
    upload.single("image1")(req, res, function (err) {
      ...
      const { name, description, price, brand, color, size, quantity } =
        req.body;
      ...
      const result = db.query(
        `INSERT INTO products (name, description, price, image1, brand, color, size, quantity) 
        VALUES ('${name}', '${description}', ${price}, '${image1}', '${brand}', '${color}', '${size}', ${quantity})`
      );

...
function Cart({ products, updateCart }) {
  ...
  const handleQuantityChange = async (product_id, quantity) => {
    ...
    try {
      const response = await axios.post(
        `https://${process.env.REACT_APP_API_URL}:${process.env.REACT_APP_API_PORT}/cart/updateProductCart`,
        {
          product_id,
          quantity,
        },
  ...
  const handleRemoveProduct = async (productId, size) => {
    try {
      const response = await axios.post(
        `https://${process.env.REACT_APP_API_URL}:${process.env.REACT_APP_API_PORT}/cart/removeProductCart`,
        {
          productId,
          size,
        },
